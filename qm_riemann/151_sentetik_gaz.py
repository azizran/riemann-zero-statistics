"""
151 — TAÇ SINAV: SENTETİK GAZ — TEORİ BAŞTAN SONA (30 Ağu)
==========================================================================
...
SONUÇ (30 Ağustos, ilk koşu) — NİTEL ZAFER, NİCEL EKSİK (dürüst):
...
# merdiven (τ ≤ 0.90)
lad = []
for p in primerange(2, int(np.exp(0.90 * L_hedef)) + 1):
    q, m = p, 1
    while q <= int(np.exp(0.90 * L_hedef)):
...
for it in range(12):
    ...
    adim = np.clip(0.7 * F / payda, -0.5 * gbar_t, 0.5 * gbar_t)
    z = np.clip(z - adim, 100.0, None)
    mf = float(np.max(np.abs(F)))
    print(f"  Newton {it}: maks|F|={mf:.2e}", flush=True)
    if mf < 1e-4:
        break
z = np.sort(z)
