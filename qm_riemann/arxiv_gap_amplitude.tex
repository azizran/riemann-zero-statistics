...
\]
(constant-model $\chi^2/\mathrm{dof} = 0.34$; a linear trend improves
$\chi^2$ by only $3.0$), while the Pearson-based shift is \emph{not}: it rises from $+0.75$ to
...
corrections through its own projection. The two effective dimensions are
not the same quantity and differ numerically by a factor of about five
over our range (at $L = 11.98$: $L/\sqrt{12\Lambda} = 2.76$ versus the
correlation-calibrated $N_{\mathrm{eff}} = 13.4$); ours is defined only
through the $r$-statistic. Third, we deliberately refrain
...
(Anthropic). An independent re-implementation of the core measurements, written from scratch with a different AI assistant (DeepSeek) and run on the raw Odlyzko tables (repository folder \texttt{bagimsiz\_\allowbreak dogrulama/}) reproduced the correlation table in
$12/12$ windows to within $0.004$ and the Gaussian-surrogate null
($0.4938 \pm 0.0059$). Errors are the author's.
...
J. London Math. Soc. (2) \textbf{112} (2025), e70250, arXiv:2411.05573.
