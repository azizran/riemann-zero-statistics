...
normalization of $0.95$--$1.1$ whose exact value is frame-dependent at
the $5$--$15\%$ level; (ii) the content splits between a
...
--- tighter than any CUE in the sampled range $N \in [5, 20]$ --- with the Pearson/Spearman
...
its validated range to sixteen orders of magnitude in $t$. At single-gap scale, two thirds or more of the apparent randomness of
...
all $\log p^k$; for the $\le 23$-frequency bases below the placebo changes the
correlations by $\sim 10^{-4}$; for the larger exhaustion bases the floor
grows to the $10^{-3}$ level and results are quoted against it.
...
of the maxima tracks the explicit-formula weight, which itself spans a
factor of $20$ ($0.71$ down to $0.035$). In the midpoint frame
$|u_q| = 1.01$ to $1.12$. The sine components vanish (phase-aligned
waves), and placebo frequencies give $|u| \approx 0$.
\end{observation}

The overall normalization carries a \emph{frame systematic}: the gap
midpoints are themselves displaced by the prime waves (the $v$ channel of
the next section), so they carry phase jitter (rms $\approx 0.14$, a
quarter of the mean gap). Re-evaluating the phases at jitter-free
positions --- the Riemann--von Mangoldt flow started at the window's first
zero --- moves every prime to $|u_p| = 0.95$--$0.97$ with \emph{no}
frequency drift (the apparent $+1\%$ to $+12\%$ drift of the midpoint
frame is a frame artifact), while the low-weight prime powers become
noisier but remain consistent with unity. We therefore state the sum rule
as: term-by-term proportionality to $p^{-k/2}/k$ is frame-robust; the
overall level is $\approx 1$ within a $5$--$15\%$ frame systematic, the
jitter-free frame preferring $\approx 0.96$ --- intriguingly close to the
$\tau \to 0$ transmission limit measured below.
...
channels are roughly consistent with the sum rule through the
gap--amplitude coupling: $w_p + \beta v_p$ lies within $5$--$30\%$ of
unity (the discrepancy growing with $p$), with $\beta \approx 2.3$ the
...
frequencies; placebo floor at the $10^{-3}$ level for these large bases,
against which the effects below remain large), the stripped correlation
...
this role-crossover $\tau^* \approx 0.40$.
\end{observation}
...
\emph{every} variant below unity, though at varying strength --- the
cleanest single-prime variant ($p=2$ only) is compatible with unity at
$1.5\sigma$, while the multi-prime variants exclude it at $4.7\sigma$ or
more:
...
frequencies tested (uncertainties $\pm 0.02$ at $q=2$ growing to
$\pm 0.2$ at the largest $q$; the $q=49$ value $1.18$ is $0.9\sigma$
from unity).
...
The gap-channel $\tau$-law is thereby verified over \emph{sixteen orders
of magnitude} in height (from $t \sim 10^5$, the lowest window where the
gap channel was measured, to $1.4\times10^{21}$). We also note the clean linear onset
...
$\tau^* \approx 0.40$, and $V_{\mathrm{res}}(L)$ --- each stable across a
factor $13$ in height within the primary windows (and far beyond in the
out-of-sample tests), each protected by placebo controls.
...
All measurements are scripts 27--60 in
