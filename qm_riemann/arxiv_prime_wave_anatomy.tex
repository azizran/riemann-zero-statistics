...
regressions, ground-truth-validated at $15$--$25\%$ on the amplitude
channel --- and quotes all channel numbers in the corrected
...
normalization near unity whose exact value carries frame and basis
systematics at the $5$--$15\%$ level, both identified and controlled in
this revision --- under the complete-basis convention the prime powers
fall on the same transmission curve as the primes; (ii) the content splits between a
...
any CUE in the sampled range $N \in [5, 20]$ --- with the Pearson/Spearman
...
shows a four-prime basis under-measures $w$ by $15$--$25\%$
($21\sigma$), while a complete basis (all $p^k$ with
$\log p^k / L \le 0.45$--$0.55$, capped at $q \le 720$) recovers the
truth to $\sim 1\%$. All channel coefficients in this revision use the
complete-basis convention; correlation statistics ($r^*$) and
regression-free quantities are insensitive to it, and the companion
note's results involve no channel regressions at all.
...
jitter-free frame preferring $\approx 0.96$. Under the complete-basis
convention the picture sharpens: the prime-power contents
$u(4) = 0.700$, $u(8) = 0.560$, $u(9) = 0.530$, $u(25) = 0.338$,
$u(27) = 0.335$ each coincide, within errors, with the prime
transmission curve $w(\tau)$ evaluated at the line's own
$\tau = \log p^k / L$: term-by-term proportionality to $p^{-k/2}/k$
holds with a \emph{single} $\tau$-dependent transmission factor common
to primes and prime powers.
...
version were $5$--$20\%$ lower in $w$ and higher in $|v|$)
...
note, sits at $0.447 \pm 0.005$ --- within $1.5\sigma$ of this coarse
...
$w_2 = 0.910 \pm 0.004$ at $\tau = 0.028$ ($L = 24.48$), and a crude
linear extrapolation gives $w(0) \approx 0.97$--$0.98$.
...
points to $w(0) \approx 0.97$--$0.98$. Whether a small residual
...
$q=2$ growing to $\pm 0.2$ at the largest $q$).
...
shallower onset (slope $\approx 1.6$ for $\tau < 0.2$), whose exact
...
absorption $1 - w(0)$, now bounded near $0.02$--$0.03$ by the corrected
trace, via the five-model re-extrapolation;
